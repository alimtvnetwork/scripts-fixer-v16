...
specific versioned repository (e.g. `scripts-fixer-v15`), the bootstrap should
**transparently redirect to the newest published version** of the repo
(e.g. `scripts-fixer-v15`) instead of installing a stale generation.
...
```
github.com/<owner>/scripts-fixer-v15
github.com/<owner>/scripts-fixer-v15
...
github.com/<owner>/scripts-fixer-v15      <-- current
github.com/<owner>/scripts-fixer-v15      <-- not yet created
```
...
```
  ...
  [REDIRECT] Switching to scripts-fixer-v15...
...
- [ ] Run from `scripts-fixer-v15` when only v5 exists → runs self
- [ ] Run from `scripts-fixer-v15` when v7 exists → redirects to v7
- [ ] Run from `scripts-fixer-v15` (latest) → runs self with "[OK] You're on the latest"
...
When copying `install.ps1` and `install.sh` into a new `-vN` repository (e.g., `scripts-fixer-v15`), update these values before committing:
